...
// Backend/routes/attendance.js (MySQL Version)
...
router.post('/', (req, res) => {
    ...
    if (!records || !Array.isArray(records) || records.length === 0) {
        return res.status(400).json({ "error": "Invalid or empty records data." });
    }

    // Prepare data for a bulk "upsert" (INSERT...ON DUPLICATE KEY UPDATE)
    const values = records.map(record => [record.student_id, date, record.status]);
    ...
    // The '?' in the VALUES clause will be replaced by the array of arrays
...
// --- PUT route to calculate overall attendance for all students ---
router.put('/recalculate-overall', (req, res) => {
    // This query is more efficient in MySQL. It calculates a summary of percentages
    // and then joins it to the students table to update the attendance column.
    const sql = `
        UPDATE students s
        JOIN (
            SELECT 
                student_id,
                (SUM(CASE WHEN status IN ('Present', 'Late') THEN 1 ELSE 0 END) / COUNT(id)) * 100 AS percentage
            FROM attendance_records
            GROUP BY student_id
        ) AS ar_summary ON s.id = ar_summary.student_id
        SET s.attendance = ar_summary.percentage;
    `;

    db.query(sql, (err, results) => {
        if (err) return res.status(500).json({ "error": err.message });
        res.json({ "message": "Overall attendance percentages recalculated.", "changes": results.affectedRows });
    });
});
